...
# Light theme with high-contrast black text (badges keep their own colors)
st.markdown(
    """
    <style>
    /* Global background and high-contrast text (avoid overriding .tag so badges keep colors) */
    body, .stApp {
        background-color: #F8F9FB;
        color: #000000;
    }

    /* Force black text for common content areas (avoid generic span so badges stay colored) */
    .block-container, p, li, label, h1, h2, h3, h4, .kpi, .kpi-label, .explain, .card,
    .st-dh, .st-dp, .st-bo, .stMarkdown, .stText, .stTextInput, .stNumberInput, .stDataFrame {
    ...
    /* Tabs styling */
    .stTabs [data-baseweb="tab-list"] { gap: 12px; margin-top: 20px; }
    ...
    .stTabs [data-baseweb="tab"]:hover { background-color: #CFD8DC; }
    ...
    .kpi-label { font-size: 14px; margin-top: -4px; font-weight: 500; }

    /* Explanation text */
    .explain { font-size: 1rem; line-height: 1.6rem; margin-bottom: 1rem; }

    /* Tag/Badge styling — DO NOT force color so inline badge colors show */
    .tag {
...
if not MODEL_PATH.exists():
    st.warning("Model not found. Training automatically... please wait ⏳")
    try:
        subprocess.run([sys.executable, str(BASE / "train.py")], check=True)
        st.success("✅ Model training completed successfully. Loading model...")
        st.rerun()  # auto-refresh so the model loads immediately
    except subprocess.CalledProcessError as e:
        st.error(f"Training failed. Please check train.py. Details: {e}")
        st.stop()
...
with tabs[0]:
    ...
    st.markdown(
        "<div class='explain'>Select a location and click <b>Load Defaults</b> to view the area's current metrics "
        "and auto-fill the numeric drivers with median values.</div>",
        unsafe_allow_html=True
    )

    if model is None:
        st.error("Prediction model could not be loaded even after training. Please check your pipeline and try again.")
    else:
        ...
        state_val = c_loc1.selectbox(STATE_COL, state_opts, index=initial_state_idx, key="pred_state_key")
        ...
        dist_val = c_loc2.selectbox(DISTRICT_COL, dist_opts, index=initial_dist_idx, key="pred_district_key")
        ...
        # Current metrics (after loading defaults)
        st.write("---")
        ...
        if not pd.isna(current_percentage):
            ...
            driver_data = {c: f"{current_defaults.get(c, 0.0):.2f}" for c in key_num}
            display_df = pd.DataFrame(list(driver_data.items()),
        ...
        if go:
            ...
            row.update(edits)
            Xnew = pd.DataFrame([row], columns=[c for c in df.columns if c not in set(filter(None, [label_col, used_percent_col]))])
            try:
                ...
                proba_df = pd.DataFrame({"Stage": model.classes_, "Probability": pred_proba}) \
                ...
                st.caption("All non-visible categorical and numeric features were auto-filled "
                           "with the selected location's mode/median to ensure a complete input row.")
            except Exception as e:
                if "tabulate" in str(e).lower():
                    st.error("Prediction failed: Missing optional dependency 'tabulate'. Please run 'pip install tabulate' and restart the app.")
                else:
...
with tabs[1]:
    ...
    st.markdown(
        ...
        "Categories are derived from the <b>Groundwater Extraction Percent</b> "
        "(<b>&lt;70%</b> Safe, <b>70–90%</b> Semi-critical, <b>90–100%</b> Critical, <b>&gt;100%</b> Over-exploited).</div>",
        unsafe_allow_html=True
    )
    ...
    if label_col:
        ...
        st.caption("Bigger orange/red slices indicate a higher share of high-stress groundwater areas.")
    else:
...
with tabs[3]:
    ...
    with c1:
        st.markdown(
            ...
            "<li>Mandate <b>rooftop rainwater harvesting</b> in new buildings.</li>"
        ...
        st.markdown(
            ...
            "<ul>"
            "<li>Promote <b>drip/sprinkler irrigation</b> and crop diversification.</li>"
    ...
    with c2:
        st.markdown(
            ...
            "<ul>"
            "<li>Limit new deep borewells; <b>mandatory metering</b> for large users.</li>"
        ...
        st.markdown(
            ...
            "<ul>"
            "<li><b>Strict abstraction controls</b> and permits for all users.</li>"
